...
def count_empty_seats(chart)
  ...
  empty_seats = 0
  chart.each do |row|
    row.each { |seat| empty_seats += 1 if seat == nil }
  end
  empty_seats
end
...
def find_favorite(array_of_hash_objects)
  ...
  if array_of_hash_objects.none? { |hash| hash[:is_my_favorite?] == true }
    nil
  else
  array_of_hash_objects.each do |hash|
    hash.each { |k, v| if k == :is_my_favorite? and hash[k] == true then return hash end }
    end
  end
end
